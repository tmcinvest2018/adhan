...
export const fetchSurahList = async (): Promise<Surah[]> => {
  ...
  try {
    ...
    // Return fallback list so the app is usable
...
export const fetchAvailableTafsirEditions = async (language: string): Promise<TafsirEdition[]> => {
    // Return restricted list as per user request: Ibn Kathir, Jalalayn, Tabari, Qurtubi.
    return [
...
export const fetchSurahContent = async (
  ...
  try {
    ...
    console.error("Quran API Error, using fallback content for demo", error);
    ...
    // Minimal Fallback for other Surahs to prevent crash
    return [{
        number: 1,
...
export const fetchAyahTafsir = async (surahNumber: number, ayahNumber: number, editionId: string = 'en.ibnkathir'): Promise<string> => {
    ...
    // 2. English Fallback
    // Check if we already tried the main English fallback
    ...
    // 3. Arabic Fallback
    // Check if we already tried the main Arabic fallback
...
export const searchAyahs = async (query: string, language: string): Promise<SearchResult[]> => {
    ...
    try {
        ...
        console.warn("API Search failed, using local fallback search");
        
        // --- FALLBACK SEARCH (Local) ---
        // Search in FALLBACK_FATIHA_AYAHS and FALLBACK_SURAHS
        ...
        // 2. Search Al-Fatiha Text (Mock Data)
...
export const RECITERS = [
    ...
    { id: 'ar.saadalkhamdi', name: 'Saad Al-Ghamdi' }, // Correct ID for Islamic Network API might vary, defaulting to known working one or similar
