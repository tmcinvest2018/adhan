...
import { Surah, Ayah, LastRead, QuranContext, AppSettings } from '../types';
...
export const QuranModule: React.FC<Props> = ({ t, language, settings, initialContext }) => {
  // --- STATE ---
  ...
  const openSurah = async (surah: Surah, startAyahIndex: number = 0) => {
    ...
    try {
        const data = await fetchSurahContent(surah.number, settings.reciterId, transId);
        setAyahs(data);
    } catch (err) {
  ...
  const handleNextSurah = () => {
    if (currentSurah && currentSurah.number < 114) {
      const next = surahList.find(s => s.number === currentSurah.number + 1);
  ...
  const handlePrevSurah = () => {
    if (currentSurah && currentSurah.number > 1) {
      const prev = surahList.find(s => s.number === currentSurah.number - 1);
