...
import { UkhuwwahModule } from './components/UkhuwwahModule'; // IMPORTED
import { PrayerDetailModal } from './components/PrayerDetailModal';
import { Onboarding } from './components/Onboarding';
import { Compass, Clock, Settings as SettingsIcon, MapPin, Loader2, Hourglass, GraduationCap, ArrowLeft, BarChart3, Navigation, Users } from 'lucide-react'; // Added Users icon
...
export default function App() {
  const [activeTab, setActiveTab] = useState<'prayers' | 'qibla' | 'ilmhub' | 'quran' | 'tracker' | 'ukhuwwah'>('prayers');
  ...
  const [settings, setSettings] = useState<AppSettings>(() => {
      ...
      if (!parsed.quranVisuals) {
          parsed = { ...parsed, quranVisuals: DEFAULT_SETTINGS.quranVisuals };
      }
      return parsed;
  });
  ...
  const dateLocale = LOCALE_MAP[settings.language] || locales.enUS;
  ...
  }, [settings.useGPS, settings.manualCoordinates, onboarded]);
                    ...
                    <div className="flex flex-col gap-0.5 mb-1">
                        <p className="text-emerald-100 text-[10px] font-bold uppercase tracking-wider">{t.status.nextPrayer}</p>
                         <div className="flex items-center text-emerald-200 text-[10px] gap-1 opacity-90 font-medium italic">
                            {settings.useGPS ? <Navigation size={10} /> : <MapPin size={10} />}
  ...
  return (
    <div className={`h-screen flex flex-col bg-[#f0fdf4] ${isRtl ? 'rtl' : 'ltr'} overflow-hidden w-full`} dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="flex-none p-4 pb-safe-area-bottom w-full max-w-lg mx-auto bg-[#f0fdf4] z-20">
        <div className="bg-white/90 backdrop-blur-md border border-gray-200/50 rounded-2xl shadow-xl shadow-gray-200/50 p-1.5 flex justify-between gap-1 overflow-x-auto no-scrollbar">
            <button 
                onClick={() => setActiveTab('prayers')}
                className={`flex-1 flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 py-3 px-2 rounded-xl transition-all duration-300 min-w-[50px] ${activeTab === 'prayers' ? 'bg-emerald-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-100'}`}
            >
                <Clock size={20} />
                <span className="font-medium text-[10px] sm:text-xs hidden sm:inline">{t.tabs.prayers}</span>
            </button>

            <button 
                onClick={() => setActiveTab('tracker')}
                className={`flex-1 flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 py-3 px-2 rounded-xl transition-all duration-300 min-w-[50px] ${activeTab === 'tracker' ? 'bg-emerald-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-100'}`}
            >
                <BarChart3 size={20} />
                <span className="font-medium text-[10px] sm:text-xs hidden sm:inline">{t.tabs.tracker}</span>
            </button>
            
            <button 
                onClick={() => setActiveTab('ilmhub')}
                className={`flex-1 flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 py-3 px-2 rounded-xl transition-all duration-300 min-w-[50px] ${activeTab === 'ilmhub' ? 'bg-emerald-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-100'}`}
            >
                <GraduationCap size={20} />
                <span className="font-medium text-[10px] sm:text-xs hidden sm:inline">{t.ilmhub.title.split(' ')[0]}</span>
            </button>
            
            <button 
                onClick={() => setActiveTab('qibla')}
                className={`flex-1 flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 py-3 px-2 rounded-xl transition-all duration-300 min-w-[50px] ${activeTab === 'qibla' ? 'bg-emerald-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-100'}`}
            >
                <Compass size={20} />
                <span className="font-medium text-[10px] sm:text-xs hidden sm:inline">{t.tabs.qibla}</span>
            </button>

            {/* NEW UKHUWWAH TAB */}
            <button 
                onClick={() => setActiveTab('ukhuwwah')}
                className={`flex-1 flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 py-3 px-2 rounded-xl transition-all duration-300 min-w-[50px] ${activeTab === 'ukhuwwah' ? 'bg-emerald-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-100'}`}
            >
                <Users size={20} />
                <span className="font-medium text-[10px] sm:text-xs hidden sm:inline">{t.tabs.ukhuwwah}</span>
            </button>
        </div>
      </div>
